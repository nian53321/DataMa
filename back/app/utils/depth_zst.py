...
v2 压缩策略（体积约为 v1 的 1/4 ~ 1/3）：
  - 量化：z16 值按 quantum=32mm 步长 round-to-nearest 量化（先 clamp 到
    65504mm 再加半步长右移），量化误差 ±16mm；QUANTUM 调为 1 即回到无损模式。
    远距（>2m）该误差低于 D455 深度噪声（2-5m 处约 ±5-15mm）；
    近距离（<1m，噪声 ±1-3mm）误差超过噪声，近距人脸几何测量请勿依赖毫米值。
    步长档位（真实录制 1280x800@30fps 离线重编实测，相对 16mm 基准）:
      16mm → 基准 @ 8.85%（占原始体积）；32mm → -30.2%；
      48mm → -43.5%；64mm → -51.8%
    （旧档案记录 16mm 为 9.1%~9.9%、8mm 为 12.2%~13.2%，与本次 16mm 实测吻合）
  - 帧间差分：**实测在这类内容上基本失效**（2026-09-16，同一份样本 895 帧中
    856 帧为关键帧，39 个差分帧全部是逐像素完全重复的帧、合计仅 3,198 B）。
    原因是相邻帧约 46% 像素深度值变化、有效像素中 79.5% 在变、平均幅度 76-88mm，
    差分后反而膨胀到 >关键帧，因而每帧都触发 DELTA_FORCE_RATIO 回退。
    ⇒ KEYFRAME_INTERVAL 只在"近乎静止的场景"下才有意义，不要把压缩率寄托在它上面；
    该路径保留仅为兼容静态场景（此时差分极有效）。
...
MAGIC = b"DZST"
VERSION = 2
# 量化步长是压缩率的主导参数（实测档位见文件头注释）。
# 2026-09-16 起 16 → 32：真实录制（video_..._B01_face，895 帧 1280x800）离线重编
# 实测体积 -30.2%（每帧 189,486 → 132,337 B），代价是量化误差 ±8mm → ±16mm。
# 选 32 而非 48/64 的依据：样本平均深度 2.9m，D455 在 >2m 的噪声本身已达 ±5-15mm，
# 32mm 档位（±16mm）仍与该噪声同量级；再放粗则量化误差将超过传感器噪声成为主导误差。
# ⚠️ 若下游要做近距（<1.5m）人脸几何精确测量，把这里调回 16 或更小。
QUANTUM = 32            # 量化步长（z16 单位，D455 默认 1mm/单位即 32mm；须为 2 的幂）
QMAX = 65535 // QUANTUM     # 量化值上限（QUANTUM=1 时即为无损模式）
CLAMP_MAX = QMAX * QUANTUM  # 量化前 clamp 上限（65504mm，覆盖 D455 全量程）
KEYFRAME_INTERVAL = 30  # 关键帧间隔（30fps 下约 1 秒）
DELTA_FORCE_RATIO = 0.5  # 差分帧压缩后超过关键帧大小该比例时强制回退关键帧
# ← 下面一步只有 QUANTUM 是 2 的幂时才等价于除法。若改成 48 之类的非幂值，
#   >> _QSHIFT 会算出错误的量化索引，文件不报错但深度值全错（解码端无从发现），
#   因此这里运行时强校验而非只写注释。
if QUANTUM & (QUANTUM - 1):
    raise ValueError("depth_zst.QUANTUM 必须是 2 的幂，当前=%d" % QUANTUM)
