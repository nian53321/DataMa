...
v2 压缩策略（体积约为 v1 的 1/4 ~ 1/3）：
  - 量化：z16 值按 quantum=16mm 步长 round-to-nearest 量化（先 clamp 到
    65520mm 再加半步长右移），量化误差 ±8mm。远距（>2m）低于 D455 深度噪声
    （2-5m 处约 ±5-15mm）；近距离（<1m，噪声 ±1-3mm）误差略超噪声，
    但人脸分析以彩色为主、深度仅提供几何信息；QUANTUM 调为 1 即回到无损差分模式；
    步长 8mm 时误差 ±4mm、体积约大 25%（两份真实录制 1280x800@30fps 实测
    8mm 为原始体积 12.2%~13.2%、16mm 为 9.1%~9.9%，折中取 16mm）；
  - 帧间差分：相邻帧绝大多数像素差值为 0 或 ±1 量化级（深度噪声），
    zstd 对差分帧压缩率极高；固定间隔插入关键帧，差分膨胀（场景突变）
    时自适应提前回退；量化步长是压缩率的主导参数；
...
MAGIC = b"DZST"
VERSION = 2
QUANTUM = 16            # 量化步长（z16 单位，D455 默认 1mm/单位即 16mm；须为 2 的幂）
QMAX = 65535 // QUANTUM     # 量化值上限（QUANTUM=1 时即为无损模式）
CLAMP_MAX = QMAX * QUANTUM  # 量化前 clamp 上限（65520mm，覆盖 D455 全量程）
